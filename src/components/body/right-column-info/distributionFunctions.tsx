...
import { Plan, Course, Major, UserCourse } from '../../commonTypes'

import { getCourse } from '../../assets'
...
export const updateFulfilled = (requirements: [string, requirements[]][], courses: UserCourse[], allCourses: Course[]) => {
  console.log(courses);
  console.log(requirements);
  courses.forEach((course) => {
    const courseObj = getCourse(course.number, allCourses);
    ...
    for (let i = 0; i < requirements.length; i++) {
      for (let j = 0; j < requirements[i][1].length; j++) {
        ...
        if (checkRequirementSatisfied(splitRequirements(requirement.expr), courseObj)) {
...
const checkRequirementSatisfied = (splitArr: string[], course: Course):boolean => {
  let boolExpr = "";
  let index = 0;
  //const course = getCourse(className, allCourses);
  if (course === null) {
    return false;
  }
  let concat;
  ...
  major.distributions.forEach(element => {
    ...
    if (element.fine_requirements !== undefined) {
      element.fine_requirements.forEach(fine => {
        allReq = [...allReq, {name: fine.description.toString(), expr: fine.criteria.toString(), required_credits: fine.required_credits, fulfilled_credits: 0}]
      })
    }
